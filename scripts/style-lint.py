    ...
    if sev("kitchen") != "off" and "/docs/" not in path.replace("\\", "/"):
        KITCHEN = [
            ...
            (r"(?<![А-Яа-яЁё])(удочк|зацепк|затравк|тизер|болванк|врезк)\w*",
             ...
             None, "warn"),
            # Номер параграфа в тексте — приглашение прыгнуть. Пишем направление.
            # Заголовок самого параграфа «# § 1.1. …» — законный, его пропускаем.
            (r"^(?![ \t]*#).*?§\s*\d+\.\d+",
             "номер параграфа в тексте: нужно направление, а не адрес",
             None, "warn"),
            # Читатель разговорника нов в 1С: опыта в ней у него предполагать нельзя.
            (r"(?<![А-Яа-яЁё])вы (?:пользуетесь|привыкли|знаете|писали|делали)\b[^.!?]{0,60}",
             "предполагает опыт читателя в 1С — а он тут новичок",
             r"Python|JavaScript|C#|в своём языке|снаружи", "warn"),

...
| [5. Процедуры и функции](chapters/05_procedury_i_funkcii/05-01_procedura_i_funkciya.md) | Процедура и функция, `Знач`, значения по умолчанию, `Экспорт`, обработчики событий | draft |
| [6. Обработка ошибок](chapters/06_obrabotka_oshibok/06-01_popytka_isklyuchenie.md) | `Попытка`, отсутствие `finally` и фильтра по типу, `ВызватьИсключение`, транзакции | draft |
| [7. Асинхронность](chapters/07_asinhronnost/07-01_otkuda_vzyalas.md) | Модальность и веб-клиент, `ОписаниеОповещения`, `Асинх`/`Ждать`/`Обещание`, фоновые задания | draft |
| [8. Объекты вместо классов](chapters/08_obekty/08-01_klassov_net.md) | Ссылка и объект, менеджер, `Новый`, структура вместо литерала, где лежит код | draft |
